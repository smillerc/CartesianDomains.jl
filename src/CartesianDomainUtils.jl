...
export shift, from_lower, from_upper
...
function from_lower(domain::CartesianIndices{N}, axis::Int, n::Int) where {N}
...
function from_upper(domain::CartesianIndices{N}, axis::Int, n::Int) where {N}
...
function get_halo_and_donor_regions(
  full_domain::CartesianIndices{N}, axis::Int, nhalo::Int
) where {N}
  domain = expand(full_domain, -nhalo)
  lo = from_lower(domain, axis, nhalo)
  hi = from_upper(domain, axis, nhalo)

  dlo = shift(lo, axis, +nhalo)
  dhi = shift(hi, axis, -nhalo)

  return (halo=(; lo, hi), donor=(lo=dlo, hi=dhi))
end
